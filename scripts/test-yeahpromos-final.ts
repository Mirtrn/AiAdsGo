  ...
  while (true) {
    ...
    try {
      ...
      if (status === 'completed' || status === 'failed') {
        ...
        console.log(`   Final URL Suffix: ${data.final_url_suffix || data.finalUrlSuffix || ''}`)
        console.log(`   分类: ${data.category || 'N/A'}`)
        console.log(`   描述: ${(data.brand_description || data.brandDescription || '').substring(0, 300)}...`)
        console.log('='.repeat(80))

        // 验证结果
        if (finalUrl.includes('diamondsfactory.ca') && brand !== 'Unknown') {
          console.log('\n✅ 测试成功: Final URL正确，品牌已提取!')
        } else if (finalUrl.includes('diamondsfactory.ca') && brand === 'Unknown') {
          console.log('\n⚠️  URL解析成功但品牌未提取')
        } else if (!finalUrl.includes('diamondsfactory.ca')) {
          console.log('\n❌ URL解析失败')
        }
...
async function main() {
  console.log('🚀 最终测试YeahPromos URL (Offer创建后完整验证)')
  console.log('='.repeat(80))
  console.log('')
