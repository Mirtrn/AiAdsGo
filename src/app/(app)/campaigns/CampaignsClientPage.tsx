...
export default function CampaignsClientPage({
  ...
  const stackedCostMetrics: TrendChartMetric[] = trendsCurrencies.map((currency, idx) => ({
    key: `cost_${currency}`,
    label: `花费 ${currency}`,
    color: costStackPalette[idx % costStackPalette.length],
  ...
  const handleRenameCampaign = async () => {
    ...
    try {
      ...
      setCampaigns(prev =>
        prev.map(c => c.id === renameTarget.id ? { ...c, campaignName: trimmed } : c)
      )
      // 同步更新批量操作快照中的名称，避免批量操作时显示旧名称
      // 注意：需在 setCampaigns 外部调用，不能在 state updater 内部触发其他 setState
      if (selectedCampaignIds.has(renameTarget.id)) {
        setSelectedCampaignSnapshots(prev => {
          const existing = prev[renameTarget.id]
          if (!existing) return prev
          return { ...prev, [renameTarget.id]: { ...existing, campaignName: trimmed } }
        })
      }
                      ...
                      <TableCell className="whitespace-nowrap">
                        <div className="flex items-center gap-1.5 min-w-0 group">
                          ...
                          <button
                            ...
                            className="opacity-0 group-hover:opacity-100 transition-opacity p-0.5 rounded hover:bg-gray-100 text-gray-400 hover:text-blue-600 shrink-0"
                            title="重命名"
                            aria-label="重命名"
                          >
