...
import { approveStrategyRecommendation } from '@/lib/openclaw/strategy-recommendations'
...
export async function POST(
  ...
  try {
    const recommendation = await approveStrategyRecommendation({
      userId: auth.userId,
      recommendationId,
    })
    ...
    const message = error?.message || '审批建议失败'
