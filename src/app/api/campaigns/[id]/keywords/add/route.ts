...
function normalizeMatchType(value: unknown, keywordText: string, brand: string | null): 'BROAD' | 'PHRASE' | 'EXACT' {
  ...
  return recommendMatchTypeForKeyword({
    keyword: keywordText,
  ...
  for (const item of inputs) {
    ...
    normalized.push({
      ...
      matchType: normalizeMatchType(
        typeof item === 'string' ? undefined : item?.matchType,
        text,
        brand
      ),
    })
...
async function createKeywordsWithDuplicateTolerance(params: {
  userId: number
  customerId: string
  refreshToken: string
  adGroupId: string
  ...
  try {
    const batch = await createGoogleAdsKeywordsBatch({
  ...
  for (const item of params.keywords) {
    try {
      const single = await createGoogleAdsKeywordsBatch({
        customerId: params.customerId,
        refreshToken: params.refreshToken,
        adGroupId: params.adGroupId,
        keywords: [
...
export async function POST(
  ...
  try {
    ...
    const syncCampaignConfigKeywords = async (keywordRows: CampaignConfigKeyword[]) => {
    ...
    const parsedKeywords = parseKeywords(body.keywords, campaign.offer_brand)
    if (parsedKeywords.length === 0) {
      return NextResponse.json({ error: '请提供至少一个有效关键词' }, { status: 400 })
    }

    const existingKeywordRows = await db.query<{ keyword_text: string }>(
      `
    ...
    const existingKeywordSet = new Set(
      (existingKeywordRows || [])
        .map((row) => normalizeKeywordText(row.keyword_text).toLowerCase())
        .filter(Boolean)
    )
    ...
    for (const item of createResult.created) {
      createdKeywordLookup.set(item.keywordText.toLowerCase(), item)
    }
    const matchTypeLookup = new Map<string, 'BROAD' | 'PHRASE' | 'EXACT'>()
    for (const item of toCreate) {
      matchTypeLookup.set(item.text.toLowerCase(), item.matchType)
    }
