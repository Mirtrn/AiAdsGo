...
// ============================================
// v4.26 5+5+5 结构验证和重新排序 (2025-12-26)
// ============================================

/**
 * 标题分类类型
 */
type HeadlineCategory = 'model' | 'brand' | 'scenario'

/**
 * 5+5+5 验证结果
 */
interface FiveByFiveFiveResult {
  valid: boolean
  categories: {
    model: string[]
    brand: string[]
  ...
  issues: string[]
  reorganizedHeadlines: string[]
}

/**
 * 判断标题是否包含产品型号
 * 规则：包含完整产品型号（如 "Qrevo Curv 2 Pro", "Gen 2", "Air 3"）
 */
function containsProductModel(headline: string, productName: string): boolean {
  if (!productName) return false

  // 提取产品名中的型号部分（通常是连续的单词）
  const modelPatterns = [
    // 常见型号格式
    /\b[A-Z][a-z]+\s+[A-Z][a-z]+(?:\s+Pro)?(?:\s+\d+)?(?:\s+Pro)?/i,  // "Qrevo Curv 2 Pro"
    /\b[A-Z][a-z]+\s+\d+(?:\s+Pro)?(?:\s+\d+)?/i,  // "Air 3 Pro"
    /\b[A-Z][a-z]+\s+[A-Z][a-z]+/i,  // "Gen 2" (两个词)
    /\b[A-Z][a-z]+\d+\b/i,  // "S8", "Q5"
    /\b[A-Z]{2,}\d+\b/i,  // "HS30"
  ]

  // 检查是否包含产品名
  if (headline.toLowerCase().includes(productName.toLowerCase())) {
    return true
  }
...
/**
 * 判断标题是否为品牌+品类聚焦
 * 规则：包含品牌名 + 品类词，但不包含具体型号
 */
function isBrandCategoryFocus(headline: string, brandName: string, category: string): boolean {
  const headlineLower = headline.toLowerCase()
  const brandLower = brandName.toLowerCase()
  const categoryLower = category.toLowerCase()

  // 必须包含品牌名
  if (!headlineLower.includes(brandLower)) {
    return false
  }

  // 常见品类词
  const categoryKeywords = [
    'robot', 'vacuum', 'cleaner', 'aspirateur', 'store', 'shop',
    'camera', 'security', 'ring', 'watch', 'headphones', 'earbuds',
    'pro', 'sale', 'discount', 'official', 'officiel'
  ]

  // 如果包含产品型号，不算品牌品类聚焦
  if (containsProductModel(headline, brandName)) {
    return false
  }

  // 包含品牌 + 至少一个品类词
  return categoryKeywords.some(keyword => headlineLower.includes(keyword))
}

/**
 * 判断标题是否为场景+功能聚焦
 * 规则：聚焦使用场景或核心功能，可能不提品牌
 */
function isScenarioFunctionFocus(headline: string, brandName: string): boolean {
  const headlineLower = headline.toLowerCase()
  const brandLower = brandName.toLowerCase()

  // 如果包含品牌名，需要同时包含场景/功能词
  if (headlineLower.includes(brandLower)) {
    const scenarioKeywords = [
      'pet', 'animal', 'home', 'house', 'family', 'baby', 'child',
      'for', 'avec', 'pour', 'chez', 'auto', 'automatique',
      'solution', 'nettoyage', 'cleaning', 'protection'
    ]
    const functionKeywords = [
      'suction', 'aspiration', 'power', 'puissance', 'heat', 'chaud',
      'lift', 'wireless', 'smart', 'intelligent', 'auto', 'wash',
      'mop', 'cordless', 'fast', 'quick', 'easy', 'simple'
    ]
    return scenarioKeywords.some(k => headlineLower.includes(k)) ||
           functionKeywords.some(k => headlineLower.includes(k))
  }

  // 如果不包含品牌名，检查是否为纯场景/功能描述
  const standalonePatterns = [
    /\d+\s*Pa/i,  // "25000 Pa"
    /heat\s*\d/i, /température/i,  // 温度相关
    /wireless/i, /cordless/i,  // 无线
    /solution/i, /system/i,  // 系统/方案
  ]

  return standalonePatterns.some(p => p.test(headline))
}

/**
 * 5+5+5 结构验证和重新排序
 * 确保 15 个标题符合 5+5+5 结构，并根据桶类型调整优先级
 *
 * @param headlines 原始标题数组（15个）
 * @param bucketType 桶类型 ('A'|'B'|'C'|'D'|'S')
 * @param brandName 品牌名
 * @param productName 产品名（用于识别型号）
 * @param category 品类名
 * @returns 验证结果和重新排序后的标题
 */
export function validateAndReorganize55Structure(
  headlines: string[],
  bucketType: 'A' | 'B' | 'C' | 'D' | 'S',
  brandName: string,
  productName: string,
  category: string
): FiveByFiveFiveResult {
  const issues: string[] = []
  const categories: { model: string[]; brand: string[]; scenario: string[] } = {
    model: [],
    brand: [],
    scenario: []
  }
  ...
  for (const headline of headlines) {
    if (containsProductModel(headline, productName)) {
      categories.model.push(headline)
    } else if (isBrandCategoryFocus(headline, brandName, category)) {
      categories.brand.push(headline)
    } else if (isScenarioFunctionFocus(headline, brandName)) {
      categories.scenario.push(headline)
    } else {
      // 无法分类的标题，默认放入 brand（假设包含品牌）
      if (headline.toLowerCase().includes(brandName.toLowerCase())) {
        categories.brand.push(headline)
      } else {
        categories.scenario.push(headline)
      }
  ...
  // 2. 统计和报告
  const totalCategorized = categories.model.length + categories.brand.length + categories.scenario.length
  if (totalCategorized !== headlines.length) {
    issues.push(`⚠️ 标题分类不完整: ${headlines.length} 个输入, ${totalCategorized} 个分类`)
  }

  // 3. 根据桶类型确定各分类的目标数量
  const targets = getCategoryTargets(bucketType, categories)

  // 4. 检查各分类数量是否符合要求
  if (categories.model.length !== targets.model) {
    issues.push(`📊 产品型号聚焦: ${categories.model.length}/${targets.model} (目标: ${targets.model})`)
  }
  if (categories.brand.length !== targets.brand) {
    issues.push(`📊 品牌品类聚焦: ${categories.brand.length}/${targets.brand} (目标: ${targets.brand})`)
  }
  if (categories.scenario.length !== targets.scenario) {
    issues.push(`📊 场景功能聚焦: ${categories.scenario.length}/${targets.scenario} (目标: ${targets.scenario})`)
  }

  // 5. 重新组织标题：确保每类恰好5个，不足的从其他类补充
  const reorganized = reorganizeTo55Structure(headlines, bucketType, brandName, productName, category)

  // 6. 计算是否有效（允许一定偏差）
  const isValid = categories.model.length >= 3 && categories.brand.length >= 3 && categories.scenario.length >= 3

  return {
    valid: isValid,
    categories,
    issues,
    reorganizedHeadlines: reorganized
  }
}
 ...
 * 根据桶类型获取各类别目标数量
 */
function getCategoryTargets(bucketType: 'A' | 'B' | 'C' | 'D' | 'S', current: { model: string[]; brand: string[]; scenario: string[] }) {
  // 默认 5+5+5 结构
  let targets = { model: 5, brand: 5, scenario: 5 }

  // 根据桶类型调整优先级
  switch (bucketType) {
    case 'A': // 品牌认知 - 全部需要包含品牌
      targets = { model: 5, brand: 5, scenario: 5 }
      break
    case 'B': // 场景导向 - 场景优先
      targets = { model: 4, brand: 5, scenario: 6 }
      break
    case 'C': // 功能导向 - 功能优先
      targets = { model: 5, brand: 4, scenario: 6 }
      break
    case 'D': // 价格促销 - 价格优先
      targets = { model: 4, brand: 5, scenario: 6 }
      break
 ...
 * 重新组织标题以符合 5+5+5 结构
 */
function reorganizeTo55Structure(
  headlines: string[],
  bucketType: 'A' | 'B' | 'C' | 'D' | 'S',
  brandName: string,
  productName: string,
  ...
  for (const headline of headlines) {
    if (containsProductModel(headline, productName)) {
      categorized.model.push(headline)
    } else if (isBrandCategoryFocus(headline, brandName, category)) {
      categorized.brand.push(headline)
    } else if (isScenarioFunctionFocus(headline, brandName)) {
      categorized.scenario.push(headline)
    } else {
      // 无法分类的放入 brand
      if (headline.toLowerCase().includes(brandName.toLowerCase())) {
        categorized.brand.push(headline)
  ...
  categorized.model = unique(categorized.model)
  categorized.brand = unique(categorized.brand)
  categorized.scenario = unique(categorized.scenario)

  // 获取目标数量
  const targets = getCategoryTargets(bucketType, categorized)

  // 按目标数量添加各类标题
  // 顺序：model → brand → scenario（确保多样性）
  for (const headline of categorized.model.slice(0, targets.model)) {
    result.push(headline)
  }
  for (const headline of categorized.brand.slice(0, targets.brand)) {
    result.push(headline)
  }
  for (const headline of categorized.scenario.slice(0, targets.scenario)) {
    result.push(headline)
  }

  // 如果不足15个，补充（使用品牌词扩展）
  while (result.length < 15) {
    const brandHeadline = generateBrandVariant(brandName, category, result.length)
    result.push(brandHeadline)
  }
 ...
 * 生成品牌变体标题（用于补充不足的标题）
 */
function generateBrandVariant(brandName: string, category: string, index: number): string {
  const variants = [
    `${brandName} Official Store`,
    `${brandName} ${category} Sale`,
    `Buy ${brandName} Online`,
    `${brandName} Best Deals`,
    `${brandName} Shop Now`,
    `${brandName} ${category} Expert`,
    `${brandName} Quality Guaranteed`,
    `Discover ${brandName}`,
    `${brandName} for Everyone`,
    `Explore ${brandName}`
  ]
...
/**
 * 应用 5+5+5 验证到创意数据
 * 在 buildAdCreativePrompt 解析完成后调用
 */
export function apply55StructureValidation(
  headlines: string[],
  bucketType: 'A' | 'B' | 'C' | 'D' | 'S',
  brandName: string,
  productName: string,
  category: string
): { headlines: string[]; validationReport: string } {
  if (headlines.length !== 15) {
    console.warn(`⚠️ 标题数量不为15个 (${headlines.length}), 跳过5+5+5验证`)
    return { headlines, validationReport: '跳过：标题数量不为15' }
  }

  const result = validateAndReorganize55Structure(
    headlines,
    bucketType,
    brandName,
    productName,
  ...
  // 生成验证报告
  let report = `📊 5+5+5 结构验证 (桶${bucketType}):\n`
  report += `   产品型号聚焦: ${result.categories.model.length}个\n`
  report += `   品牌品类聚焦: ${result.categories.brand.length}个\n`
  report += `   场景功能聚焦: ${result.categories.scenario.length}个\n`
  ...
  if (result.valid) {
    ...
    report += `   ⚠️ 验证未完全通过，已重新组织`
  }

  console.log(report)

  return {
    headlines: result.reorganizedHeadlines,
...
export async function generateAdCreative(
  ...
  // 🔥 v4.26 新增：5+5+5 结构验证和重新排序
  if (result.headlines.length === 15 && options?.bucket) {
    console.log(`\n📊 v4.26 5+5+5 结构验证...`)
    const productName = (offer as { product_title?: string }).product_title || ''
    const category = (offer as { category?: string }).category || ''
    const bucketType = options.bucket as 'A' | 'B' | 'C' | 'D' | 'S'

    const validationResult = apply55StructureValidation(
      result.headlines,
      bucketType,
      offerBrand,
      productName,
    ...
    // 更新 headlines
    fullResult.headlines = validationResult.headlines
    console.log(`✅ 5+5+5 验证完成: ${validationResult.headlines.length}个标题已重新组织`)
  }
