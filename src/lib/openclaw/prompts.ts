...
- Offer 创建字段规范：product_price 支持带货币的价格（如 $349.99），若只给数字则按 target_country 自动补货币符号；commission_payout 同时支持“比例”(如 30%) 与“绝对佣金”(如 $15)：若来源是佣金比例/佣金率或仅给裸数字，默认按比例处理（补成 %）；若要绝对佣金，必须显式携带货币符号。
