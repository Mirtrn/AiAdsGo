...
- Offer 创建字段规范：product_price 支持带货币的价格（如 $349.99），若只给数字则按 target_country 自动补货币符号；commission_payout 同时支持“比例”(如 30%) 与“绝对佣金”(如 $15)。
- 当来源字段是“佣金比例/佣金率”时，必须原样传比例值（建议同时提供 commission_rate，并让 commission_payout 与其一致）。例如：价格 $129.99、佣金率 12.75% 时，commission_payout 必须是 12.75%，绝不能写成 16.57%。
- 仅当来源字段明确是“佣金金额/每单佣金”且值带货币符号时，才按绝对佣金传递；若只给裸数字且无明确字段语义，默认按比例处理（补成 %）。
...
- 长耗时任务（Offer 提取、创意生成）优先使用 stream 接口（/api/offers/extract/stream/:taskId、/api/creative-tasks/:taskId/stream）持续订阅；无法使用 stream 时，再使用状态接口并采用自适应轮询间隔（建议 2-8 秒，禁止固定高频轮询）。
...
- 创意生成必须遵循 3 类型逻辑：A（品牌/信任）、B（场景+功能）、D（转化/价值·全量关键词）；禁止走旧创意接口。
...
- 当返回数据里同时存在 id 与 googleCampaignId，update-cpc 只能使用 googleCampaignId，禁止混用本地 id
