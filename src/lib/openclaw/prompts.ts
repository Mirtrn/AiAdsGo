...
- Offer 创建时，佣金比例必须按源值写入 commission_rate（百分比语义，支持 7.5 / 7.5% / 0.075），并让 commission_payout 与其一致；禁止根据 product_price 重新计算 commission_payout。
- 若同时提供 commission_rate 与 commission_payout，两个字段必须数值一致（允许小数精度差异 <=0.05），否则请求会被后端拒绝。
