-- ==========================================
-- AutoAds SQLite Schema - Consolidated Edition
-- ==========================================
...
CREATE TABLE google_ads_accounts (
  ...
  last_sync_at TEXT,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now')), parent_mcc_id TEXT, test_account BOOLEAN NOT NULL DEFAULT FALSE, status TEXT DEFAULT 'UNKNOWN', account_balance REAL DEFAULT NULL,
  FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE "offers" (
  ...
  pricing TEXT,
  is_active INTEGER DEFAULT 1,
  ai_reviews TEXT,              -- AI产品评论洞察（JSON: rating, sentiment, positives, concerns）
  ai_competitive_edges TEXT,    -- AI竞争优势（JSON: badges, primeEligible, stockStatus, salesRank）
  ai_keywords TEXT,             -- AI关键词列表（JSON数组: 产品相关关键词）
  -- v3.2优化（2025-12-08）：店铺/单品差异化分析字段
  ai_analysis_v32 TEXT,         -- v3.2差异化分析数据（JSON: storeQualityLevel, marketFit, credibilityLevel等）
...
CREATE TABLE scraped_products (
  ...
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now')), deep_scrape_data TEXT, review_analysis TEXT, competitor_analysis TEXT, has_deep_data INTEGER DEFAULT 0, product_info TEXT,
...
CREATE TABLE ad_creative_performance (
  ...
  min_clicks_reached INTEGER DEFAULT 0,
...
-- ==========================================
CREATE TABLE login_attempts ( id INTEGER PRIMARY KEY AUTOINCREMENT, username_or_email TEXT NOT NULL, ip_address TEXT NOT NULL, user_agent TEXT, success INTEGER DEFAULT 0, failure_reason TEXT, attempted_at TEXT DEFAULT (datetime('now')) );
...
CREATE TABLE risk_alerts (
  ...
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now')), alert_type TEXT, resource_type TEXT, resource_id INTEGER, details TEXT, acknowledged_at TIMESTAMP,
  FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX idx_offers_user_offer_name_unique ON offers(user_id, offer_name) WHERE is_deleted = 0;
...
-- Google Ads settings -- P0-2修复: config_key → key, config_value → value
...
-- AI settings -- P0-2修复: config_key → key, config_value → value
...
-- Proxy settings -- P0-2修复: config_key → key, config_value → value
INSERT INTO system_settings (user_id, category, key, value, data_type, is_sensitive, is_required, description)
VALUES
  (NULL, 'proxy', 'urls', NULL, 'json', 0, 0, '代理URL配置，JSON格式存储国家与代理URL的映射');

-- System settings -- P0-2修复: config_key → key, config_value → value
INSERT INTO system_settings (user_id, category, key, value, data_type, is_sensitive, is_required, default_value, description)
VALUES
